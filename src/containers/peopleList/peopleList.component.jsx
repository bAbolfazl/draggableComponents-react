...
const PeopleList = ({ setPeopleList, selectPeopleList }) => {
  useEffect(() => {
    // const x = Boolean(selectPeopleList.length);
    if (selectPeopleList.length) {
      //   debugger;
      return;
    }
  ...
  const handleDrop = (ev) => {
    const dragBox = selectPeopleList.find(
      (box) => Number(box.id) === Number(dragId)
    );
    const dropBox = selectPeopleList.find(
      (box) => Number(box.id) === Number(ev.currentTarget.id)
    );
    // debugger
